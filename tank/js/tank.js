...
var Tank = function(){
	...
	//super shoot
	this.super_shoot = function(type){
	...
	//get Bullet
	this.GetSuperBullet=function(bullet,iii){ 
			if(!iii)
				iii=1;
			var tempX = this.x;
			var tempY = this.y;
			bullet = new Bullet(this.ctx,this,type,this.dir);
			if(this.dir == UP){
				tempX = this.x + parseInt(this.size/2) - (iii * parseInt(bullet.size/2));
				tempY = this.y - iii * bullet.size ;
			}else if(this.dir == DOWN){
				tempX = this.x + parseInt(this.size/2) - (iii * parseInt(bullet.size/2));
				tempY = this.y + iii * this.size;
			}else if(this.dir == LEFT){
				tempX = this.x - iii * bullet.size;
				tempY = this.y + parseInt(this.size/2) - (iii * parseInt(bullet.size/2));
			}else if(this.dir == RIGHT){
				tempX = this.x + (iii * this.size);
				tempY = this.y + parseInt(this.size/2) - (iii * parseInt(bullet.size/2));
			}
			bullet.x = tempX;
			bullet.y = tempY;
...
var EmemyShootType=2;
var EmemyShootTime=34;
/**
 * 敌方坦克1
 * @param context 画坦克的画布
 * @returns
...
var EnemyOne = function(context){
	...
	this.draw = function(){
		...
		if(!this.isAppear){
			...
			if(this.times == EmemyShootTime){
				this.isAppear = true;
				this.times = 0;
				this.shoot(EmemyShootType);
			}
		}else{
