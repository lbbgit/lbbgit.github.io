...
//检测敌对存在
function CheckIntersectExists(object1, object2, overlap,wucha)
{
	//    x-轴                      x-轴
	//  A1------>B1 C1              A2------>B2 C2
	//  +--------+   ^              +--------+   ^
	...
	//判断y-轴重叠
		if(C1 >= C2-wucha && C1 <= D2+wucha || D1 >= C2-wucha && D1 <= D2+wucha)
		{
			if(A2>A1)
				return RIGHT;
			else
	...
	//假如他们在x-轴重叠
	if(A1 >= A2-wucha && A1 <= B2+wucha
	   || B1 >= A2-wucha && B1 <= B2+wucha)
	{
		if(C1 >= C2) 
			return UP; 
		else
